...
# Here the user enter "investment" or "bond"/ ".lower()" allow to write in upper or lower
user_select = input("Select your choice ( Investment or Bond): ").lower()

# Here this is a statement, so if user enter "investment"
if user_select == "investment":
    # User enter the amount of money
    amount_money = int(input("Enter the amount of money for your deposit: "))
    # User enter the interest rate only numbers(decimal include), we don't need to add %
    interest_rate = float(input("Enter your interest rate wish (only numbers): "))
    # User enter how many years
    years = int(input("How many years: "))
    # User choice between "simple" or "compound"/ ".lower()" allow to write in upper or lower
...
# From the first "if", if we don't enter "investment" or "bond" so "else" will be executed
else:
